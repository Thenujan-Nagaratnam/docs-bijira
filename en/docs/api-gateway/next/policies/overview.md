...
| [OAuth2 Generator](https://github.com/wso2/gateway-controllers/blob/main/docs/oauth2-generator/v0.9/docs/oauth2-generator.md) | Security, AI | Generates an upstream credential (via an OAuth2 client credentials or password grant, or a static bearer token) and injects it into a configurable request header before forwarding the request to the backend. |
